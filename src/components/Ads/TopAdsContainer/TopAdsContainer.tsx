...
const Card = ({ image,url}:{image:string,url:string}) => {
    ...
    return (
        ...
        <article style={{ position:"relative"}}>
            <img className={"responsive"} src={image} style={{position:"absolute" , top:0,left:0,right: 0 ,bottom:0}}/>
...
export const TopAdsContainer = () => {
    ...
    useEffect(() => {
        Ex.apiClient().get("/api/ad/first/1").then(value => setAds(value.data)).catch( reason => setAds(null) )
    },[setAds])
