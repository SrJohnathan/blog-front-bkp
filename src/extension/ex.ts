export namespace Ex {
  ...
  export  const convertItem = <T>(item: any): T => {
    ...
    export const modeTheme = () : {t: string, b: boolean} => {
        ...
        // @ts-ignore
        let newMode = ui("mode") == "dark" ? {t: "light", b: false} : {t: "dark", b: true};
        // @ts-ignore
        ui("mode", newMode.t);
    ...
    export const modeThemeBackInit = ():{t: string, b: boolean}  => {
    ...
    export const isThemeBackInit = ():{t: string, b: boolean}  => {
   ...
   export function replaceNumberWithText(input: string, textMappings: string[]): string {
